...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel corner of 3x4 image") {
    val src = new Img(3, 4)
    src(0, 0) = 0; src(1, 0) = 1; src(2, 0) = 2
    src(0, 1) = 3; src(1, 1) = 4; src(2, 1) = 5
    src(0, 2) = 6; src(1, 2) = 7; src(2, 2) = 8
    ...
    assert(boxBlurKernel(src, 0, 0, 4) === 9,
      s"(boxBlurKernel(0, 0, 4) should be 9, " +
        s"but it's ${boxBlurKernel(src, 0, 0, 4)})")
  }
...
//    VerticalBoxBlur.blur(src, dst, 0, 4, 2)
    VerticalBoxBlur.parBlur(src, dst, numTasks = 2, radius = 2)
