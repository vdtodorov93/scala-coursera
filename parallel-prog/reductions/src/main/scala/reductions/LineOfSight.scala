...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    output(0) = 0
    (1 until output.length).foreach(i => {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    var i = from
    var best = input(from) / from
    while(i < until) {
      if(input(i) / i > best) best = input(i) / i
      i = i + 1
    }
    best
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = {
    (from until until).foreach(i => {
      if(i == 0) output(i) = 0
      else if(i == from) output(i) = (input(i) / i).max(output(i - 1))
      else output(i) = (input(i) / i).max(output(i - 1))
    })
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float, tree: Tree): Unit = {
    tree match {
      case Leaf(from, until, maxPrevious) => downsweepSequential(input, output, maxPrevious max startingAngle, from, until)
