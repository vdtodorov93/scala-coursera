...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.groupBy(e => e).map(e => (e._1, e._2.length)).toList.sortBy(e => e._1)
  }
...
//    val res = for{(ch, count) <- occurrences }
//      yield combinations(subtract(occurrences, (ch, count) :: Nil))
//    res.toList.flatten
    occurrences :: occurrences.flatMap(occ => combinations(subtract(occurrences, (occ._1, 1) :: Nil)))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    x.map({case (ch, count) => (ch, count - yMap(ch))}).filter({case (_, count) => count > 0})
  }
